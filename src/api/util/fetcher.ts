...
const SingletonFactory = (function () {
    ...
    const axiosInstance = axios.create({
        baseURL: ElavonEnvironment.baseUrl,
    ...
    if (ElavonEnvironment.getAuthorizationEnabled) {
    ...
    function createAxiosResponseInterceptor() {
        const interceptor = axiosInstance.interceptors.response.use(
            ...
            (error: any) => {
                ...
                const extras = ElavonEnvironment.getExtras();
                ...
                if (ElavonEnvironment.getAuthorizationEnabled && logActivity) {
                    logActivity({
                        data: { error: error },
                ...
                if (ElavonEnvironment.getAuthorizationEnabled) {
                    const url = new URL('connect/token', ElavonEnvironment.getAuthUrl());
                    const auth = Buffer.from(`${ElavonEnvironment.getMerchantId()}:${ElavonEnvironment.getMerchantPIN()}`).toString("base64");
                    const requestConfig = {
                        url: url.href,
                        method: RequestMethod.POST,
                        data: `grant_type=client_credentials&client_id=${ElavonEnvironment.getMerchantId()}&client_secret=${ElavonEnvironment.getVendorId()}&scope=gateway`,
                        headers: {
...
const fetcher = async ({ url = '', method = 'post', data = {}, params = {}, headers = {}, cookies = {}, baseUrl = "", }: any) => {
    const computedUrl = new URL(url, baseUrl || ElavonEnvironment.getBaseUrl());
    const config: any = {
        method: method,
